...
export const postBookingS = async (
  propId: string,
  userId: string,
  bookingDetail: Partial<BookingInput>
): Promise<boolean> => {
  try {
    const {
      startDate,
...
export const getMyBookingS = async (
  ...
  try {
    ...
    const reservations = await bookingModel
      ...
      .skip((page! - 1) * limit!)
      .limit(limit!)
      .populate("propertyId", "-tennants")
      .populate("paymentId")
      .populate("hostId", "_id userName profileImg userId")
      .sort({ createdtAt: -1 })
      .exec();
    if (!reservations) throw new Error("Failed to Fetch reservation for user");
    return reservations;
...
export const getOnBookingS = async (
  hostId: string,
  page?: number,
  limit?: number
): Promise<Partial<IBooking>[]> => {
  try {
    const reservations = await bookingModel
      ...
      .limit(limit!)
      .populate("propertyId")
      .populate("userId", "_id userName profileImg userId")
      .populate("paymentId")
      .sort({ createdtAt: -1 })
      .exec();
    if (!reservations) throw new Error("Failed to Fetch reservation for user");
...
export const confirmCheckInS = async (
  userId: string,
  bookingId: string
): Promise<boolean> => {
  try {
    ...
    if (booking.checkInStatus)
      throw new Error("Check in status True Verified!");
    ...
    const booked = await propertyModel.findOneAndUpdate(
    ...
    if (!booked)
      throw new Error("failed to Chnage the booking status of the property");
...
export const confirmCheckOutS = async (
  userId: string,
  bookingId: string
): Promise<boolean> => {
  try {
    ...
    const tennantExist = await propertyModel.findOne({
      _id: booking.propertyId,
      tennants: booking.userId,
    });
    if (tennantExist) return true;

    const newTennant = await propertyModel.findOneAndUpdate(
      { _id: booking.propertyId },
      {
        $push: {
          tennants: booking.userId,
