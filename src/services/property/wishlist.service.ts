...
export const addPropertyWishS = async (
  userId: string,
  id: string
): Promise<boolean> => {
  try {
    ...
    if (checkpropertyOwner)
      throw new Error(
        "Property owner not allowed add owned property in wishlist"
      );

    //check whether this user already has added property to wishli
    const checkWishList = await userModel.findOne({
      _id: userId,
      wishList: id,
    });
    ...
    if (!updateWishList)
      throw new Error("Property failed to add in the wish List");
    return true;
  } catch (e) {
    console.log(e);
    throw e;
...
export const getWishListS = async (
  ...
  try {
    ...
    if (!wishList)
      throw new Error("Wish List not avialable for the given user");
    return wishList;
  } catch (e) {
    console.log(e);
    throw e;
...
export const removePropertyS = async (
  userId: string,
  id: string
): Promise<boolean> => {
  try {
    ...
    if (!propertyExist)
      throw new Error(
        "Invalid property Id/Property with the given id does not Exist!"
      );

    const removeProperty = await userModel.findOneAndUpdate(
      { userId },
      {
        $pull: {
          wishList: id,
    ...
    if (!removeProperty)
      throw new Error(
        "Property remove Failed/Invalid property Credential for the user!"
      );
    return true;
  } catch (e) {
    console.log(e);
