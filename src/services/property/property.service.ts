...
export const getPropertyByIdS = async (
  id: string,
  userId: string
): Promise<{ property: Property; user: string; inWishList: boolean }> => {
  try {
    if (userId !== "") {
      ...
      const propertyData = await propertyModel
        .findOne({ _id: id })
        .populate("userId", "_id userName profileImg createdAt")
        .populate("tennants", "_id  userName profileImg");
      ...
      const inWishList =
        userdocument!.wishList?.some((wish) => wish.toString() == id) ?? false;

      //delete tennant informations in next js server if the user is not owner
      if (
        propertyData.tennants.some(
          (tennant) => tennant._id?.toString() == userId
        )
      ) {
        copyData.tennants = [{}];
      ...
      if (propertyData.userId._id?.toString() == userId)
        return { property: propertyData, user: "owner", inWishList };
...
export const getMyPropertiesS = async (
  page: string,
  limit: string,
  userId: string
): Promise<Property[]> => {
...
export const getPropertiesS = async (
  page: string,
  limit: string,
  ...
  try {
    ...
    if (queryParams.maxRate > 0) {
      matchConditions.rate = {
        $gte: queryParams.minRate,
        $lte: queryParams.maxRate,
      };
    }
...
export const updatePropertyS = async (
  ...
  try {
    ...
    if (!checkProperty)
      throw new Error("Invalid request credential/unAuthorized user");
    ...
    const updatedProperty = await propertyModel.findOneAndUpdate(
      { _id: id, userId },
      {
        ...
        isVerified: {
          status: false,
          pending: true,
          message: "Update needs reverification",
        },
      },
      { new: true }
    );
    ...
    const notify = sendMail(
      userPropTemplate(
        ...
        updatedProperty.images[0].imgUrl
      )
    );
...
export const deletePropertyS = async (
  ...
  try {
    const checkProperty = await propertyModel.findOne({
      _id: propertyId,
      userId,
    });
    if (!checkProperty)
      throw new Error("Invalid Delete Request/No property with the User exist");

    //check if booking for the property exist if then cant delete
    const currentDate = new Date();
    const bookingExist = await bookingModel
      .findOne({
        propertyId,
        status: "Booked",
        $or: [
    ...
    const decreaseCount = await userModel.updateOne(
      { userId },
      {
        $inc: {
          listingCount: -1,
    ...
    if (!decreaseCount)
      throw new Error("Propety deleted failed to decrease listing count");
...
export const updateViewCountS = async (
  userId: string,
  propertyId: string
): Promise<boolean> => {
  try {
    ...
    if (userId !== "") {
      ...
      const propertyOwner = await propertyModel.findOne({
        _id: propertyId,
        userId,
      });
      if (propertyOwner) return false;

      const checkViewed = await userModel.findOne({
        _id: userId,
      ...
      const updateViewedProperty = await userModel.findOneAndUpdate(
        { _id: userId },
        { $push: { viewedProperty: propertyId } },
        { new: true }
      );
    ...
    const updateViewCount = await propertyModel.updateOne(
      { _id: propertyId },
      {
        $inc: {
          viewCount: 1,
