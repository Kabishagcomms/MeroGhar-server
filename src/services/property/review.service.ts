...
export const postReviewS = async (
  ...
  try {
    ...
    //if not tennant
    if (propertyExist.tennants.some((tennant) => tennant != userId))
      throw new Error("No user allowed to Review Except tennant!");
    ...
    if (!bookingCount)
      throw new Error(
        "User Has not Completed any bookings/not Eligible to provide review"
      );

    //now get review count:
    let reviewCount = await reviewModel.countDocuments({
      userId,
      propertyId: id,
    });
    if (!reviewCount) {
    ...
    if (reviewCount >= bookingCount)
      throw new Error("Tennant already has provided Review For the booking");

    const newReview = await reviewModel.create({
      userId,
      propertyId: id,
      ...reviewData,
      hostId: propertyExist.userId,
    });
    ...
    if (propertyExist.avgRating == 0 && propertyExist.ratingCount == 0) {
      ...
      propertyExist
        ...
        .set(
          "avgRating",
          Math.round((propertyExist.avgRating + reviewData.rating!) / 2) < 0
            ? 0
            : Math.round((propertyExist.avgRating + reviewData.rating!) / 2)
        );
      await propertyExist.save();
    }
...
export const updateReviewS = async (
  ...
  try {
    ...
    if (user) {
      const updatedAvgRating = Math.round(
        ((user.avgRating - oldReview.rating) * 2 + reviewData.rating!) / 2
      );
...
export const deleteReviewS = async (
  ...
  try {
    ...
    const newAvgRating = Math.round(
      (property.avgRating - oldReview.rating) * 2
    );
    ...
    if (user) {
      const updatedAvgRating = Math.round(
        (user.avgRating - oldReview.rating) * 2
      );
      user.avgRating = updatedAvgRating < 0 ? 0 : updatedAvgRating;
      user.recievedReviewcount = user.recievedReviewcount - 1;
      await user.save();
