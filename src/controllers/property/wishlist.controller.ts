...
import {
  ...
  getWishListS,
  removePropertyS,
} from "../../services/property/wishlist.service";

export const addPropertyWishC = async (req: Request, res: Response) => {
  try {
    const addWishList = await addPropertyWishS(
      req.userData.docId,
      req.params.id
    );
    if (addWishList)
      return res
        ...
        .json({
          success: true,
          message: "property added to wish list successfully",
        });
  } catch (e: any) {
    console.log(e);
...
export const removeProperyC = async (req: Request, res: Response) => {
  try {
    const propertyRemove = await removePropertyS(
      req.userData.userId,
      req.params.id
    );
    if (propertyRemove)
      return res
        ...
        .json({
          success: true,
          message: `Property ${req.params.id} removed successfully from the wishlist`,
        });
  } catch (e: any) {
    console.log(e);
    res.status(400).json({ success: false, error: e.message });
