...
export const createPropertyC = async (req: Request, res: Response) => {
  try {
    ...
    if (!req.userData.kycVerified)
      return res
        .status(403)
        .json({ success: false, error: "Kyc not Verified/Unauthorized user" });
    ...
    if (newProperty)
      return res
        ...
        .json({
          success: true,
          message: "Property sent for further verification",
        });
  } catch (e: any) {
    console.log(e);
...
export const getPropertyByIdC = async (req: Request, res: Response) => {
  try {
    ...
    const propertyData = await getPropertyByIdS(
      req.params.id,
      req.userData.docId
    );
    return res.status(200).json({ success: true, propertyData });
...
export const getPropertiesC = async (req: Request, res: Response) => {
  try {
    ...
    const propertyData = await getPropertiesS(
...
export const getMyPropertiesC = async (req: Request, res: Response) => {
  try {
    const propertyData = await getMyPropertiesS(
      req.query.page as string,
      req.query.limit as string,
      req.params.id
    );
  ...
  try {
    ...
    if (!req.userData.kycVerified)
      return res
        .status(401)
        .json({ success: false, error: "Kyc not Verified/Unauthorized user" });
    const updatedProperty = await updatePropertyS(
      req.userData.docId,
      req.params.id,
      req.body
    );
    return res
      ...
      .json({
        success: true,
        message: "Property updated and  sent for further verification",
        updatedProperty,
      });
  } catch (e: any) {
    console.log(e);
...
export const deletePropertyC = async (req: Request, res: Response) => {
  try {
    ...
    const deleteProperty = await deletePropertyS(
      req.userData.docId,
      req.params.id
    );
    if (deleteProperty)
  ...
  try {
    ...
    if (req.userData.userId == "") {
      ...
      if (updateViewCount)
        return res
          .status(200)
          .json({ success: true, message: "view count updated" });
    }

    //since user is authenticated
    const updateViewCount = await updateViewCountS(
      req.userData.docId,
      req.params.id
    );
    if (updateViewCount)
      return res
        ...
        .json({
          success: true,
          message: "view count updated/user recent product view Updated",
        });
    ...
    return res
      ...
      .json({
        success: false,
        message: "view count not updated/user has already viewd product",
      });
  } catch (e: any) {
    console.log(e);
    res.status(400).json({ success: false, error: e.message });
